...
.result-box{align-self:center;min-height:166px;border-radius:14px;background:rgba(255,255,255,.96);border:2px solid #d0dde4;box-shadow:0 4px 12px rgba(0,0,0,.17);padding:13px 15px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;transition:background .18s ease,border-color .18s ease,box-shadow .18s ease}.result-box.result-win{background:#effcf3;border-color:#40a866;box-shadow:0 0 0 3px rgba(64,168,102,.13),0 5px 14px rgba(0,0,0,.18)}.result-box.result-loss{background:#fff1f3;border-color:#cf4f67;box-shadow:0 0 0 3px rgba(207,79,103,.12),0 5px 14px rgba(0,0,0,.18)}.result-box.result-tie{background:#fff9e8;border-color:#b58c27;box-shadow:0 0 0 3px rgba(181,140,39,.12),0 5px 14px rgba(0,0,0,.18)}.result-head{font-size:17px;font-weight:1000;color:#173d61;margin-bottom:6px;letter-spacing:.02em}.result-head.win{color:#0f6a32;font-size:20px}.result-head.loss{color:#a4163a;font-size:19px}.result-head.tie{color:#705b12;font-size:18px}.capture-note{font-size:11px;font-weight:900;color:#294e68;margin:1px 0 7px}.result-detail{font-size:10px;line-height:1.35;color:#40586d;max-width:410px}.next-note{font-size:9px;color:#72879a;margin-top:7px}.war-pot{display:flex;flex-direction:column;align-items:center;justify-content:center;color:#fff;font-weight:950;font-size:9px;gap:3px}.pot-count{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#7c5a22;border:2px solid #d8b66a;font-size:14px;box-shadow:0 2px 7px rgba(0,0,0,.25)}
.hand-area{position:relative;display:grid;grid-template-columns:1fr 150px;gap:10px;align-items:end;margin-top:11px}.hand-wrap{min-width:0}.hand-title{font-size:11px;font-weight:950;color:#e1f0f5;margin:0 0 6px 3px}.hand{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px;height:clamp(165px,21vw,215px)}.hand-slot{min-width:0;min-height:0}.hand .card{height:100%;aspect-ratio:auto}.hand .card-title{font-size:clamp(9px,.92vw,12px);height:25px;top:6px}.hand .structure-frame{top:34px;bottom:32px}.hand .pka{bottom:6px}.player-piles .pile{color:#d7eaf5}.footer-note{text-align:center;color:#6f8b9b;font-size:9px;padding:0 12px 11px}.credits{margin:12px 0 0;padding:10px 13px;border:1px solid #c7d6df;border-radius:12px;background:#f5f9fb;color:#17394f;font-size:12px;line-height:1.5;box-shadow:0 5px 16px rgba(0,0,0,.18)}.credits b{color:#073f67}.credits a{color:#16659a;text-decoration:none}.credits a:hover,.credits a:focus-visible{text-decoration:underline}.gameover{display:none;position:absolute;z-index:10;inset:0;background:rgba(4,13,20,.91);backdrop-filter:blur(5px);align-items:center;justify-content:center;padding:25px}.gameover.show{display:flex}.gameover-card{max-width:480px;width:100%;text-align:center;border:2px solid #57788d;border-radius:19px;background:linear-gradient(180deg,#173044,#0c1d29);box-shadow:0 24px 50px rgba(0,0,0,.5);padding:28px}.gameover-title{font-size:34px;font-weight:1000;margin-bottom:8px}.gameover-sub{color:#a8c1d0;line-height:1.45}.gameover-actions{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:18px}.primary{border:2px solid #915d12;background:linear-gradient(180deg,#ffe297,#dfa938);color:#352308;border-radius:12px;padding:12px;font-weight:1000;cursor:pointer}
...
.strategy-feedback{margin-top:8px;border-radius:999px;padding:6px 10px;font-size:10.5px;line-height:1.2;font-weight:900;letter-spacing:.02em}.strategy-feedback.good{border:1px solid #4d8d67;background:#e3f7e9;color:#14532d}.strategy-feedback.bad{border:1px solid #5d6c79;background:#253746;color:#f4f8fb}.strategy-feedback.neutral{border:1px solid #9c7c2d;background:#fff4cf;color:#6d540b}.strategy-feedback .strategy-label{font-weight:1000}
...
<main id="app">
  ...
  <div class="machine">
    ...
    <div class="lb-overlay" id="leaderboardModal" role="dialog" aria-modal="true" aria-labelledby="leaderboardTitle">
      <div class="lb-card" tabindex="-1" id="leaderboardCard">
        ...
        <div class="lb-foot">Strategy ranks optimal-move percentage and requires at least 20 scored moves. Tied tricks are neutral and are not included in strategy scoring. Scored moves are recorded as they are played, including moves from unfinished games. A win counts only when a game is completed.</div>
      </div>
    </div>
  ...
  <div class="credits" aria-label="Credits">
    <div><b>Design &amp; development:</b> Michael Singleton, François Kerff</div>
    <div><b>Software development:</b> AI-assisted development using ChatGPT (OpenAI)</div>
    <div><b>Scientific content &amp; educational design:</b> Michael Singleton, François Kerff</div>
    <div><b>pKa values provided by:</b> Daniel Singleton (Texas A&amp;M University)</div>
    <div><b>Problems / contact:</b> <a href="mailto:m.singleton@uclouvain.be">m.singleton@uclouvain.be</a></div>
  </div>
</main>
...
function resolveWinner(a,b){
  if(!a||!b)return'tie';
  if(a.pka!=null&&b.pka!=null){const d=Math.abs(Number(a.pka)-Number(b.pka));if(d<=TIE_TOL+1e-12)return'tie';return Number(a.pka)<Number(b.pka)?'player':'computer'}
  const av=cardValue(a),bv=cardValue(b);if(Math.abs(av-bv)<1e-9)return'tie';return av<bv?'player':'computer';
}
function wouldWin(c,opp){return resolveWinner(c,opp)==='player'}
function analyzeOptimal(cards,opp,chosenId){
  ...
  if(!candidates.length)return{optimal:true,best:[],canWin:false,scored:true};
  const bestValue=Math.max(...candidates.map(cardValue));const best=candidates.filter(c=>Math.abs(cardValue(c)-bestValue)<1e-9);
  return{optimal:best.some(c=>c.instance_id===chosenId),best,canWin:winners.length>0,scored:true};
}
function strategyHtml(a){
  if(!a)return'';
  if(a.scored===false)return'<div class="strategy-feedback neutral"><span class="strategy-label">TIE — STRATEGY NOT SCORED</span></div>';
  if(a.optimal)return'<div class="strategy-feedback good"><span class="strategy-label">🎯 OPTIMAL MOVE</span></div>';
  return'<div class="strategy-feedback bad"><span class="strategy-label">STRATEGY: NOT OPTIMAL</span></div>';
}
...
function renderIdle(){renderPiles();const cpu=$('cpuCard');cpu.innerHTML='';const node=cardNode(cpuCurrent,'cpu',false,false);node.classList.add('deal');cpu.appendChild(node);setTimeout(()=>node.classList.remove('deal'),350);const pc=$('playerCenter');pc.innerHTML='';pc.appendChild(cardNode(null));const rb=$('resultBox');rb.classList.remove('result-win','result-loss','result-tie');rb.innerHTML='<div class="result-head">Computer has played</div><div class="result-detail">Choose the acid you want to commit. If you can win, conserve strength by using the weakest acid that still wins; if you cannot win, sacrifice your least acidic card.</div>';const h=$('hand');h.innerHTML='';hand.forEach(c=>{const slot=document.createElement('div');slot.className='hand-slot';slot.appendChild(cardNode(c,'hand',false,true));h.appendChild(slot)});while(h.children.length<HAND_SIZE){const slot=document.createElement('div');slot.className='hand-slot';slot.appendChild(cardNode(null));h.appendChild(slot)}sfxDeal();flushPendingMoves();announce('Computer played '+(cpuCurrent?cpuCurrent.name:'an acid')+'. Choose a card from your hand.');setHeight()}
function renderResult(player,opp,winner,delta,captureCount,analysis){renderPiles();const cpu=$('cpuCard');cpu.innerHTML='';const cn=cardNode(opp,'cpu',true,false);cn.classList.add('flip');cpu.appendChild(cn);const pc=$('playerCenter');pc.innerHTML='';const pn=cardNode(player,'player-center',true,false);pn.classList.add('flip');pc.appendChild(pn);const h=$('hand');h.innerHTML='';hand.forEach(c=>{const slot=document.createElement('div');slot.className='hand-slot';const n=cardNode(c,'hand',false,false);n.classList.add('locked');slot.appendChild(n);h.appendChild(slot)});while(h.children.length<HAND_SIZE){const slot=document.createElement('div');slot.className='hand-slot';slot.appendChild(cardNode(null));h.appendChild(slot)};sfxFlip();setTimeout(()=>{cn.classList.remove('flip');pn.classList.remove('flip');if(winner==='player'){pn.classList.add('win');cn.classList.add('loss')}else if(winner==='computer'){cn.classList.add('win');pn.classList.add('loss')}else{cn.classList.add('tie');pn.classList.add('tie')}},640);const rb=$('resultBox');let head='',cls='',capture='',detail='';rb.classList.remove('result-win','result-loss','result-tie');if(winner==='player'){head='TRICK WON';cls='win';rb.classList.add('result-win');capture='You capture '+captureCount+' card'+(captureCount===1?'':'s')+'.';detail=pkaText(player)+' vs '+pkaText(opp)+' — '+(delta==null?'the reference table ranks your acid as stronger.':'lower pKa wins.');if(captureCount>=6)sfxCaptureBig();else sfxWin();sparks(pn)}else if(winner==='computer'){head='TRICK LOST';cls='loss';rb.classList.add('result-loss');capture='CPU captures '+captureCount+' card'+(captureCount===1?'':'s')+'.';detail=pkaText(player)+' vs '+pkaText(opp)+' — '+(delta==null?'the reference table ranks the CPU acid as stronger.':'lower pKa wins.');sfxLoss()}else{head='TIE — WAR POT';cls='tie';rb.classList.add('result-tie');capture='The pot now holds '+(warPot.length+2)+' cards. The next winner takes them all.';detail=delta==null?'Same reference acid-strength ranking.':(delta<=1e-12?'Same reference pKa.':('ΔpKa = '+Number(delta).toFixed(2)+'; differences ≤ '+TIE_TOL.toFixed(2)+' are treated as a tie.'));sfxTie()}rb.innerHTML='<div class="result-head '+cls+'">'+head+'</div><div class="capture-note">'+capture+'</div><div class="result-detail">'+detail+'</div>'+strategyHtml(analysis)+'<div class="next-note">'+(manualAdvance?'Result will remain until you choose Next trick.':'Cards move to the winner’s pile, then the next trick is dealt…')+'</div><div class="next-trick-wrap"><button type="button" class="primary next-trick-btn" id="nextTrickBtn">NEXT TRICK</button></div>';const strategyAnnouncement=analysis&&analysis.scored===false?'Tie not scored for strategy.':(analysis&&analysis.optimal?'Optimal move.':'Move was not optimal.');announce(head+'. '+capture+' '+detail+' '+strategyAnnouncement);setHeight()}
...
function playCard(id,clicked){
  ...
  const snapshot=hand.slice(),opp=cpuCurrent,analysis=analyzeOptimal(snapshot,opp,id);
  waiting=true;const player=hand.splice(idx,1)[0];cpuCurrent=null;lastPlayed={player,opp};const numericBoth=(player.pka!=null&&opp.pka!=null);const delta=numericBoth?Math.abs(Number(player.pka)-Number(opp.pka)):null;const winner=resolveWinner(player,opp);
  if(winner==='tie'){analysis.scored=false}else{gameMoves++;if(analysis.optimal)gameOptimal++;recordPersistentMove(gameMoves,analysis.optimal)}
  const captureCount=winner==='tie'?0:warPot.length+2;renderResult(player,opp,winner,delta,captureCount,analysis);if(timer)clearTimeout(timer);timer=null;const next=$('nextTrickBtn');if(next)next.addEventListener('click',()=>{if(timer){clearTimeout(timer);timer=null}advance(player,opp,winner)});if(!manualAdvance)timer=setTimeout(()=>advance(player,opp,winner),HOLD_MS)
}
...
function finishGame(p,c){gameOver=true;renderPiles();let title,sub,won=false;if(p<=0&&c<=0){title='DRAW';sub='Both sides ran out of cards at the same time.'}else if(p<=0){title='CPU WINS';sub='The computer captured the battle deck.'}else{title='YOU WIN';sub='You captured the battle deck.';won=true}const pct=gameMoves?Math.round(1000*gameOptimal/gameMoves)/10:null;const strategySummary=gameMoves?('<b>'+gameOptimal+'/'+gameMoves+' optimal ('+pct+'%)</b>'):'<b>—</b>';$('gameoverTitle').textContent=title;$('gameoverSub').innerHTML=esc(sub)+'<br>Tricks played: '+turn+' · Strategy: '+strategySummary+' <span class="saved-note">(ties excluded)</span>.'+(playerMode==='player'?'<br><span class="saved-note">Scored moves are saved as you play; completed wins count on the Wins leaderboard.</span>':'<br><span class="saved-note">Guest game — nothing was saved.</span>');$('gameover').classList.add('show');announce(title+'. '+sub+(gameMoves?(' Strategy '+gameOptimal+' of '+gameMoves+' optimal. Ties excluded.'):' No scored strategy moves. Ties excluded.'));setTimeout(()=>$('gameoverCard').focus(),20);if(won)sfxCaptureBig();else sfxLoss();finishPersistentGame(won);setHeight()}
